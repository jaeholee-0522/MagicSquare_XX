...
| Version | 1.1 |
| Status | Implementation-Ready (DN-01~DN-03 resolved at GREEN R8) |
...
**Both attempts fail (G3 — DN-03 resolved):**
...
→ `DOMAIN-ERR-NO_VALID_PLACEMENT` (Attempt 1·2 모두 FR-04 `false`)

**Both attempts fail (policy):** FR-01 통과 행렬 중 Attempt 1·2 모두 FR-04 `false`. G3 fixture는 R5 GREEN에서 확정·고정 (`tests/conftest.py`).
...
└─────────┬─────────┘
          ...
          ▼
┌───────────────────┐
│  Control Layer    │  SolveUseCase orchestration (**required**, DN-02)
...
| Both-fail fixture 미확정 | RED-DOM-SOL-003 blocked | **Resolved (DN-03):** G3 fixture in `tests/conftest.py` |
...
**Resolved at GREEN R8 (v1.1):**

| ID | Decision | Rationale |
|---|---|---|
| **DN-01** | **(A) bare `int[6]`** | `SolveUseCase.execute` 및 U-OUT-01~03 GREEN; 성공 시 래퍼 없이 `[r1,c1,n1,r2,c2,n2]` 반환 |
| **DN-02** | **(B) Control 필수** | `SolveUseCase`가 Boundary 검증 후 Domain 위임; Boundary→Domain 직접 호출 금지 |
| **DN-03** | **G3 both-fail matrix 확정** | `[[0,14,15,4],[12,6,7,9],[8,10,11,5],[13,3,2,0]]` — D-SOL-03 GREEN |
...
*End of PRD v1.1 — R1~R6 GREEN complete; DN-01~DN-03 resolved.*
